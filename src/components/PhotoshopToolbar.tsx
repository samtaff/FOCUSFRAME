...
import { Square, Circle, Eye, EyeOff, Crosshair, GripHorizontal, Move, FolderDown } from 'lucide-react';

interface PhotoshopToolbarProps {
  focus: FocusRect;
  onUpdateFocus: (updates: Partial<FocusRect>) => void;
  screenW?: number;
  screenH?: number;
...
export const PhotoshopToolbar: React.FC<PhotoshopToolbarProps> = ({
  ...
  const isPillH = (focus.shape === 'pill' || !focus.shape) && focus.width > focus.height;
  const isPillV = (focus.shape === 'pill' || !focus.shape) && focus.height >= focus.width;
  const isCircle = focus.shape === 'circle';
  const isRounded = focus.shape === 'rounded';
  const isRectangle = focus.shape === 'rectangle';

  // Apply Pill H (Hauteur par défaut 35px)
  const selectPillH = () => {
    const pillHeightPct = (35 / screenH) * 100; // 35px de hauteur par défaut
  ...
  const selectPillV = () => {
    const pillWidthPct = (35 / screenW) * 100; // 35px
    const pillHeightPct = (128 / screenH) * 100; // 128px
  ...
  const selectRounded = () => {
    const roundedWPct = (35 / screenW) * 100; // 35px
  ...
  const selectRectangle = () => {
    const rectWPct = (35 / screenW) * 100; // 35px
  ...
  const selectCircle = () => {
    const circleSizeWPct = (55 / screenW) * 100; // 55px
    const circleSizeHPct = (55 / screenH) * 100; // 55px
    const currentCenterX = focus.x + focus.width / 2;
    const currentCenterY = focus.y + focus.height / 2;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="w-full flex flex-col items-center justify-center pt-0.5 pb-1.5 border-b border-black/[0.06] cursor-grab active:cursor-grabbing group transition-all"
      >
      ...
      >
        <div
          className={`w-2 h-4.5 rounded-full border ${
            isPillV && focus.enabled ? 'border-white bg-white/40' : 'border-slate-700'
          }`}
        />
      ...
      >
        <div
          className={`w-4.5 h-2 rounded-full border ${
            isPillH && focus.enabled ? 'border-white bg-white/40' : 'border-slate-700'
          }`}
        />
      ...
      >
        <div
          className={`w-3.5 h-4.5 rounded-md border ${
            isRounded && focus.enabled ? 'border-white bg-white/40' : 'border-slate-700'
          }`}
        />
      ...
      <button
        ...
        id="ps-tool-center"
        onClick={centerBoth}
        title="Recentrer la zone de focus (Centre X & Y)"
      ...
      <button
        ...
        onClick={() => onUpdateFocus({ showHandles: focus.showHandles === false ? true : false })}
        title={focus.showHandles !== false ? 'Masquer les poignées de redimensionnement' : 'Afficher les poignées de redimensionnement'}
        className={`relative w-8 h-8 rounded-xl flex items-center justify-center transition-all cursor-pointer active:scale-95 ${
          focus.showHandles !== false
      ...
      <button
        ...
        onClick={() => onUpdateFocus({ showBorder: !focus.showBorder })}
        title={focus.showBorder ? 'Masquer le contour' : 'Afficher le contour'}
        className={`relative w-8 h-8 rounded-xl flex items-center justify-center transition-all cursor-pointer active:scale-95 ${
          focus.showBorder
      ...
      >
        <div
          className={`w-3.5 h-3.5 rounded-sm border-2 ${
            focus.showBorder ? 'border-rose-600 bg-rose-500/30' : 'border-slate-400'
          }`}
        />
      ...
      <button
        ...
        onClick={() => onUpdateFocus({ enabled: !focus.enabled })}
        title={focus.enabled ? 'Désactiver la zone focus' : 'Activer la zone focus'}
      ...
      >
        {focus.enabled ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
